...
struct T* prove(struct file* f, struct query* q , int num_query, struct public_key* pubkey, SkipList& S)
{
 ...
 /**
   Step 1: Calculate s = summation (nu*si) mod e
           and Calculate s_prime = summation(nu*si - s)/ e
 **/
 ...
 /**
   Step 2: Compute ui = [vi ei] by augumenting vector vi with unit
 ...
 /**
    Step 3: Compute w = summation(nu*u)mod e
            and Compute w_prime = summation(nu*ui - w)/ e
 **/
